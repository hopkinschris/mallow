  ...
  def mailer_unfollower_details(size, id, options={})
    ...
    unfollower_status =
      if user_followers.include?(id)
        content_tag(:span, "Following", :class => 'status', :style => "font-size:0.8em;line-height:3em;color:#fff;padding:2px 5px;text-transform:uppercase;background-color:#6cd4ff;border-radius:3px;")
      else
        content_tag(:span, raw("<strike>Following</strike>"), :class => 'status', :style => "font-size:0.8em;line-height:3em;color:#777;padding:2px 5px;text-transform:uppercase;background-color:#EEE;border-radius:3px;")
      end
    details = 
      content_tag(:li, handle, :class => 'handle', :style => "-webkit-font-smoothing:antialiased;list-style:none;color:#444;") +
      unfollower_status
